...
test("prepareWasm copies worker helpers and writes a manifest", async () => {
    ...
    await writeFile(join(snippet, "wasm_web_start_workers.js"), "await pkg.default(init);" );

    const manifest = await prepareWasm({ outDir: root, threads: 4 });

    assert.equal(manifest.threads, 4);
    assert.equal(await readFile(join(snippet, "worker_helpers.js"), "utf8"), "await pkg.default({ memory: init.memory });" );
    assert.equal(await readFile(join(snippet, "wasm_web_start_workers.js"), "utf8"), "await pkg.default({ memory: init.memory });" );
